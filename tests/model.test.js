...
test('Invalid settings and oversized building sets fail with useful errors',async()=>{assert.throws(()=>validateModelSettings({...defaultModel(),width:NaN}),/width/);await assert.rejects(()=>buildModel(engine,{...state,osmData:{buildings:Array(3001).fill(state.osmData.buildings[0])}},defaultModel()),/3,000/);});
